...
# Godot 3 TileSet enum compatibility values for this legacy addon.
const BITMASK_2X2: int = 0
const BITMASK_3X3_MINIMAL: int = 1
const BITMASK_3X3: int = 2
const TILE_MODE_AUTO_TILE: int = 1
...
@export var autotile_mode: MODE = MODE.BITMASK_3X3_MINIMAL
@export var tile_size: Vector2 = Vector2()
@export var copy_collisions: bool = false

var _autotile_mode = BITMASK_3X3_MINIMAL
...
func setup_3x3_min() -> void:
	_autotile_mode = BITMASK_3X3_MINIMAL
	_src_tiles_count = 5
	_dest_tiles_x = 12
	_dest_tiles_y = 4
	_tiles_data.clear()
...
func create_tileset_texture(image_path: String = "") -> Texture2D:
	if !get_children():
		return null
	...
	for node in get_children():
		if node.visible and node is Sprite2D:
	...
	var tileset_image: Image = Image.create(int(texture_size.x), int(texture_size.y), false, Image.FORMAT_RGBA8)
...
func fill_tileset(tile_set: TileSet, tileset_texture: Texture2D = create_tileset_texture()) -> TileSet:
	for i in range(_sprites.size()):
...
func _blit_tileset_image(sprite: Sprite2D, tileset_image: Image, texture_offset: Vector2) -> void:
...
func _blit_shapes(sprite: Sprite2D) -> Dictionary:
	if !copy_collisions:
		return {}
...
func _create_autotile(tile_set: TileSet, tile_name: String, texture: Texture2D, texture_offset: Vector2, shapes: Dictionary = {}) -> void:
	...
	tile_set.tile_set_tile_mode(id, TILE_MODE_AUTO_TILE)
